...
/** 分析草稿：深拷贝表行；补齐缺省 startDate，避免浅拷贝/陈旧 state */
...
const InfoTip = ({ children }: { children: ReactNode }) => {
  ...
  const tipNode = open && (
    <>
      {isMobile && <button type="button" aria-label="关闭说明" className="fixed inset-0 z-[59] cursor-default bg-transparent" onClick={() => setOpen(false)} />}
      {createPortal(
        <span
          ref={tipRef}
          role="tooltip"
          className={`fixed z-[60] w-64 max-w-[calc(100vw-2rem)] rounded-xl bg-[#17212b] p-3 text-left text-xs font-normal leading-5 text-white shadow-xl ${isMobile ? '' : 'pointer-events-none'}`}
...
export default function HomePage() {
  ...
  const [snapshotSalary, setSnapshotSalary] = useState(16667);
  ...
  const askDelete = (anchor: HTMLElement | null, payload: { kind: 'expense' | 'snapshot'; id: string; title: string; message: string }) => {
  ...
  const handleAuthed = async (user: AuthUser, meta: { from: AuthMode }) => {
    ...
    try {
      ...
      // 空账号：优先当前内存画像（访客刚改的），再兜底 localStorage
      let toBind = profile;
      try {
        const saved = localStorage.getItem('money-manage-profile');
        if (saved) {
      ...
      applyProfileData(toBind as unknown as Record<string, unknown>);
  ...
  }, [hydrated, authUser, salary, cash, emergencyMonths, totalAssets, invest, investRatio, returnRate, reinvestRate, socialRates, expenses, rentEnabled, elderlyEnabled, snapshots, retirement]);
  ...
  const addExpense = () => {
    const id = uid();
    setExpenses((items) => [...items, { id, name: '新支出', category: '其他', mode: 'fixed', amount: 0, startDate: todayDateKey() }]);
    // 等 React commit；桌面 tr（md 隐藏）与移动 card 双锚点，取有布局盒的；nearest 在列表底常不滚
    window.setTimeout(() => {
  ...
  const requestRemoveSnapshot = (snapshot: Snapshot, beforeSalary: number, anchor: HTMLElement | null) => {
    const after = snapshot.changes[0]?.value ?? beforeSalary;
    askDelete(anchor, {
      kind: 'snapshot',
      id: snapshot.id,
      title: '删除快照',
      message: snapshotDeleteMessage(
        ...
        `税前工资 ${money(beforeSalary)} → ${money(after)}`,
      ),
    });
  };
    ...
    <header className="app-header page-pad mx-auto flex max-w-[1920px] items-center justify-between gap-2 border-b border-slate-100 px-3 py-2 sm:px-6 lg:px-10">
      ...
      <div className="relative flex min-w-0 flex-1 items-center justify-end gap-1.5 sm:gap-2">
        ...
        <div className="relative">
          ...
          {headerMoreOpen && (
            <>
              <button type="button" className="fixed inset-0 z-[60] cursor-default" aria-label="关闭菜单" onClick={() => setHeaderMoreOpen(false)} />
              <div className="header-more-menu absolute right-0 z-[70] mt-1.5 w-52 rounded-2xl border border-slate-100 bg-white p-1.5 shadow-lg" role="menu">
                ...
                <button
                  ...
                  onClick={() => {
                    setHeaderMoreOpen(false);
                    if (showSnapshotPanel) setShowSnapshotPanel(false);
                    else { setSnapshotSalary(salary); setSnapshotDate(today); setShowSnapshotPanel(true); }
                  }}
                >
                  快照设置{snapshots.length > 0 ? ` · ${snapshots.length}` : ''}
                </button>
                ...
                {!authUser && isNarrow && (
                  <button type="button" role="menuitem" className="header-more-item" onClick={() => { setHeaderMoreOpen(false); setAuthOpenRequest('login'); }}>登录</button>
                )}
                {authUser && (
                  <button
                    type="button"
        ...
        {showSnapshotPanel && (
          <FloatPanel open={showSnapshotPanel} anchorRef={snapshotBtnRef} onClose={() => setShowSnapshotPanel(false)} width={720} headerTitle="未来快照">
            <div className="flex items-center justify-between gap-3">
              <div><h3 className="font-semibold">未来快照</h3><p className="mt-1 text-xs text-slate-400">保存前会确认负资产击穿</p></div>
              <button type="button" onClick={() => setShowSnapshotPanel(false)} className="text-xs text-slate-400">关闭</button>
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_auto] lg:items-end">
              <label className="field"><span>快照名称</span><input value={snapshotName} onChange={(event) => setSnapshotName(event.target.value)} onBlur={saveEvent} /></label>
              <DateEditable label="生效日期" value={snapshotDate} min={today} onChange={setSnapshotDate} />
              <label className="field"><span>变化后税前工资</span><input type="number" min="0" max="2000000" value={snapshotSalary} onChange={(event) => setSnapshotSalary(Number(event.target.value))} /></label>
              <button type="button" onClick={addSnapshot} className="rounded-xl bg-[#17212b] px-4 py-2.5 text-sm font-semibold text-white">保存快照</button>
            </div>
            {snapshotMessage && <p className="mt-3 text-sm text-emerald-700">{snapshotMessage}</p>}
            {snapshots.length > 0 ? (
              <div className="mt-4 max-h-[360px] space-y-2 overflow-y-auto">
                {snapshots.map((snapshot, index) => {
                  const previous = snapshots[index - 1];
                  const before = previous?.changes[0]?.value ?? salary;
                  return (
                    <div key={snapshot.id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-slate-50 px-4 py-3 text-sm">
                      <div>
                        <strong>{snapshot.name}</strong>
                        <span className="ml-3 text-slate-500">{snapshot.effectiveDate}</span>
                        <span className="ml-3 text-slate-600">税前工资 {money(before)} → {money(snapshot.changes[0].value)}</span>
                        {snapshot.allowNegative && <span className="ml-3 text-amber-700">已确认允许击穿</span>}
                      </div>
                      <button type="button" onClick={(event) => requestRemoveSnapshot(snapshot, before, event.currentTarget)} className="text-xs text-red-500">删除</button>
                    </div>
                  );
                })}
              </div>
            ) : (
              <p className="mt-4 text-sm text-slate-400">暂无快照，填写上方信息后保存。</p>
            )}
          </FloatPanel>
        )}
      </div>
...
<div className="mt-4 space-y-3 md:hidden">{expenses.map((expense) => (
          <div key={expense.id} data-expense-anchor={expense.id} className="expense-card space-y-2">
            <div className="expense-card-head">
              <div className="expense-card-title min-w-0 flex-1">
                <ClickField label="名称" display={expense.name || '未命名'} panel={<label className="block text-xs text-slate-500">名称<input className="field-input mt-1" value={expense.name} onChange={(event) => { updateExpense(expense.id, { name: event.target.value }); saveEvent(); }} /></label>} />
              </div>
              <div className="expense-card-amount min-w-0 shrink">
                <ClickField label="金额 / 月付" display={formatExpensePayment(expense)} panel={expense.mode === 'fixed' || expense.mode === 'one_time' ? <div className="grid gap-3">{expense.mode === 'one_time' && <label className="block text-xs text-slate-500">发生时间（默认当月）<input className="field-input mt-1" type="date" value={(expense.startDate || defaultOneTimeDate()).slice(0, 10)} onChange={(event) => { updateExpense(expense.id, { startDate: event.target.value || defaultOneTimeDate(), endDate: event.target.value || defaultOneTimeDate() }); saveEvent(); }} /></label>}<label className="block text-xs text-slate-500">{expense.mode === 'one_time' ? '金额' : '每月金额'}<input className="field-input mt-1" type="number" min="0" step="100" value={expense.amount} onChange={(event) => { updateExpense(expense.id, { amount: Number(event.target.value) }); saveEvent(); }} /></label></div> : expense.mode === 'percentage' ? <label className="block text-xs text-slate-500">收入比例（%）<input className="field-input mt-1" type="number" min="0" max="100" step="1" value={expense.rate || 0} onChange={(event) => { updateExpense(expense.id, { rate: Number(event.target.value) }); saveEvent(); }} /></label> : <p className="text-xs text-slate-500">分期月付由分期信息计算</p>} />
              </div>
            </div>
            <div className="expense-card-meta">
              <ClickField label="类型" display={formatExpenseMode(expense.mode)} panel={<label className="block text-xs text-slate-500">类型<select className="field-input mt-1" value={expense.mode} onChange={(event) => { updateExpense(expense.id, { mode: event.target.value as Expense['mode'] }); saveEvent(); }}><option value="fixed">固定金额</option><option value="percentage">按比例</option><option value="installment">分期</option><option value="one_time">一次性</option></select></label>} className="expense-meta-chip" />
              <ClickField label="分类" display={expense.category || '未分类'} panel={<label className="block text-xs text-slate-500">分类<input className="field-input mt-1" value={expense.category} onChange={(event) => { updateExpense(expense.id, { category: event.target.value }); saveEvent(); }} /></label>} className="expense-meta-chip" />
            </div>
            {expense.mode === 'installment' && (
              <div className="field-row-mobile">
                <span>分期信息</span>
                <ClickField label="分期信息" display={formatExpenseInstallment(expense)} width={360} wrap density="panel" panel={<InstallmentSettingsPanel expense={expense} onChange={(patch) => { updateExpense(expense.id, patch); saveEvent(); }} retirementDate={retirement.enabled ? retirementDate : undefined} />} />
              </div>
            )}
            <div className="expense-card-actions">
              <div className="flex-1"><ExpenseAnalyzeButton expense={expense} financeInput={financeInput} reinvestRate={reinvestRate} retirementDate={retirement.enabled ? retirementDate : undefined} /></div>
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          ...
          else removeSnapshot(pendingDelete.id);
...
function ExpenseAnalyzeButton({ expense, financeInput, reinvestRate, retirementDate }: { expense: Expense; financeInput: FinanceInput; reinvestRate: number; retirementDate?: string }) {
  ...
  const anchorRef = useRef<HTMLButtonElement>(null);
  const [draft, setDraft] = useState<Expense>(() => cloneExpenseDraft(expense));
  // 打开时在 click 里同步深拷贝；关闭只收起设置。勿依赖 [expense] 异步 sync（首帧会陈旧）
  const openAnalyze = () => {
  ...
  const dirty = JSON.stringify(draft) !== JSON.stringify(cloneExpenseDraft(expense));
  const kpiCards = [
    ...
    <div className="relative inline-block">
      ...
      {open && (
        <FloatPanel open={open} anchorRef={anchorRef} onClose={closeAnalyze} width={920} maxHeightVh={90} center draggable headerTitle="消费影响分析" mode="auto" density="panel">
          ...
          <div className="mt-4 rounded-2xl border border-slate-200 bg-[#f8faf9] px-4 py-3">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <div className="flex shrink-0 items-center gap-2">
                {dirty && <button type="button" onClick={() => setDraft(cloneExpenseDraft(expense))} className="text-[11px] font-semibold text-slate-500 hover:text-[#d9654a]">重置</button>}
                <button type="button" onClick={() => setSettingsOpen((current) => !current)} className="rounded-xl bg-[#17212b] px-3 py-2 text-xs font-semibold text-white hover:bg-[#2a3644]">{settingsOpen ? '收起设置' : '测算设置'}</button>
              </div>
            </div>
            {/* 同层折叠：避免父 sheet 内再开 FloatPanel */}
            {settingsOpen && (
              <div className="mt-3 border-t border-slate-200 pt-3">
                <p className="text-[11px] text-slate-400">仅影响本行草稿，不改已保存支出。</p>
                <div className="mt-3 grid gap-3">
                  ...
                  <button type="button" onClick={() => setDraft(cloneExpenseDraft(expense))} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:border-[#f07f62] hover:text-[#d9654a]">重置为已保存</button>
                </div>
              </div>
            )}
          </div>
...
function FloatPanel({
  ...
  useEffect(() => {
    ...
    if (lockBody) document.body.classList.add('sheet-open');
  ...
  useEffect(() => {
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (target instanceof Element) {
        // 点到其它浮层：不关本层
        const otherPanel = target.closest('[data-float-panel]');
        if (otherPanel && otherPanel !== panelRef.current) return;
        // 点到更高层遮罩：不关本层（避免确认框关掉下层快照 panel）
        const backdrop = target.closest('[data-sheet-backdrop]') as HTMLElement | null;
        if (backdrop) {
          const bz = Number(backdrop.style.zIndex || 0);
          if (bz > zIndex - 1) return;
        }
      }
      onClose();
    };
  ...
  return createPortal(
    <>
      {asSheet && <div data-sheet-backdrop className="sheet-backdrop" style={{ zIndex: zIndex - 1 }} aria-hidden />}
      <div
        ref={panelRef}
        data-float-panel
        ...
        role="dialog"
        aria-modal={isPanelSheet ? 'true' : undefined}
      ...
      >
        ...
        {showHeader && (
          <div
            ...
            className={`flex shrink-0 items-center justify-between gap-3 border-b border-slate-100 px-4 ${isFieldCard ? 'py-2.5' : 'py-3'} ${draggable && !asSheet ? 'cursor-move select-none' : ''}`}
            title={draggable && !asSheet ? '按住拖动面板' : undefined}
          >
            <h3 className={`min-w-0 truncate font-semibold text-slate-800 ${isFieldCard ? 'text-sm' : 'text-base'}`}>{headerTitle || '编辑'}</h3>
            <button type="button" onClick={onClose} className={`shrink-0 rounded-full px-3 text-sm font-semibold text-slate-500 hover:bg-slate-50 hover:text-slate-700 ${asSheet ? 'touch-btn' : ''}`}>关闭</button>
          </div>
        )}
...
function Editable({ label, value, min, max, step, suffix = '', onChange }: EditableProps) {
  ...
  // 打开编辑时的原值：blur 空/非法时恢复，勿写成 0
  const baselineRef = useRef(value);
  const formatValue = (n: number) => String(Number.isInteger(n) ? n : Number(n.toFixed(1)));
  useEffect(() => { if (!open) setDraft(formatValue(value)); }, [value, open]);
    ...
    <div className="relative block">
      <span className="field-row-mobile flex items-center justify-between gap-2 text-sm text-slate-600 sm:flex-row sm:items-center">
        ...
        <button
          ref={anchorRef}
          type="button"
          onClick={() => { if (open) setOpen(false); else openEdit(); }}
      ...
      <FloatPanel open={open} anchorRef={anchorRef} onClose={() => setOpen(false)} width={280} maxHeightVh={48} headerTitle={label} density="field">
          ...
          <label className="flex-1 text-xs text-slate-500">
            ...
            <input
              ...
              onKeyDown={(event) => {
                ...
                if (event.key === 'Enter') { onDraftBlur(); setOpen(false); }
              }}
        ...
        <div className="mt-3 flex items-center gap-2">
          <input
            ...
            step="1"
            value={position}
            onChange={(event) => commit(min + (max - min) * Number(event.target.value) / 100)}
          />
          <span className="w-12 text-right text-xs text-slate-500">{position}%</span>
        </div>
      </FloatPanel>
    </div>
  );
...
/** 数值输入：允许暂时为空；blur 空/非法时恢复 focus 基线（勿写成 0） */
function SoftNumberInput({
  ...
  const finish = (raw: string) => {
    ...
    if (raw.trim() === '' || !Number.isFinite(num)) {
      setDraft(String(baselineRef.current));
      onCommit(baselineRef.current);
      return;
    }
    const next = clamp(num, min ?? Number.NEGATIVE_INFINITY, max ?? Number.POSITIVE_INFINITY);
  ...
  return (
    <input
      ...
      onChange={(event) => {
        ...
        onCommit(clamp(num, min ?? Number.NEGATIVE_INFINITY, max ?? Number.POSITIVE_INFINITY));
      }}
      onBlur={() => {
...
function AssetLinkedEditor({
  ...
  const [open, setOpen] = useState(false);
  const anchorRef = useRef<HTMLButtonElement>(null);
  const ratioLabel = Number.isInteger(investRatio) ? `${investRatio}` : investRatio.toFixed(1);
  const display = `${money(totalAssets)} · 现${money(cash)} · 理${money(invest)}（${ratioLabel}%）`;
  return (
    <div className="relative block min-w-0 sm:col-span-2">
      ...
      <FloatPanel open={open} anchorRef={anchorRef} onClose={() => setOpen(false)} width={340} maxHeightVh={56} headerTitle="资产配置" density="field">
        ...
        <label className="mb-2 block text-xs text-slate-500">总资产
          <input className="field-input mt-1" type="number" min={0} max={2000000} step={1000} value={totalAssets} onChange={(event) => onTotal(Number(event.target.value))} />
        </label>
        <div className="space-y-2">
          <LinkedFieldGroup hint="现金 + 理财 = 总资产">
            <label className="block text-xs text-slate-500">现金资产
              <input className="field-input mt-1" type="number" min={0} max={totalAssets} step={1000} value={cash} onChange={(event) => onCash(Number(event.target.value))} />
            </label>
            <label className="block text-xs text-slate-500">理财资产
              <input className="field-input mt-1" type="number" min={0} max={totalAssets} step={1000} value={invest} onChange={(event) => onInvestAmount(Number(event.target.value))} />
            </label>
          </LinkedFieldGroup>
          <LinkedFieldGroup hint="理财金额 ↔ 占比（相对总资产）">
            <label className="block text-xs text-slate-500">理财占比（%）
              <input className="field-input mt-1" type="number" min={0} max={100} step={1} value={Number.isInteger(investRatio) ? investRatio : Number(investRatio.toFixed(1))} onChange={(event) => onInvestRatio(Number(event.target.value))} />
            </label>
            <div className="flex flex-col justify-end text-xs text-slate-500">
              <span>对应理财</span>
              <span className="mt-1 font-mono text-sm font-semibold tabular-nums text-[#17212b]">{money(invest)}</span>
            </div>
          </LinkedFieldGroup>
        </div>
      </FloatPanel>
...
function ClickField({ display, panel, width = 280, wrap = false, label, className, density = 'field' }: { display: string; panel: ReactNode; width?: number; wrap?: boolean; label?: string; className?: string; density?: 'tip' | 'field' | 'panel' }) {
  const [open, setOpen] = useState(false);
  const anchorRef = useRef<HTMLButtonElement>(null);
  return <div className="relative inline-block max-w-full"><button ref={anchorRef} type="button" onClick={() => setOpen((current) => !current)} onDoubleClick={() => setOpen(true)} title="点击打开编辑" className={className ?? `field-click max-w-full text-left ${wrap ? 'field-click-wrap' : 'truncate'}`}>{display}</button><FloatPanel open={open} anchorRef={anchorRef} onClose={() => setOpen(false)} width={width} maxHeightVh={density === 'panel' ? 70 : 48} headerTitle={label} density={density}>{panel}</FloatPanel></div>;
}
