 ...
 * 退休与社保：一级入口行 → 移动同 sheet 子页 / PC 独立二级 FloatPanel，不回流财务参数主卡
 */
...
/** 一级面板内入口行 → 二级「退休与社保」弹层 */
export const INCOME_DETAIL_SOCIAL_SETTINGS_ENTRY = '退休与社保';
export const INCOME_DETAIL_SOCIAL_SETTINGS_PANEL_TITLE = '退休与社保';

/** 一级面板「本月预估个税」旁入口 → 二级个税明细（抵扣 → 分档税额 → 税率表） */
 ...
 * socialSettings：二级弹层内容（规划基数与收入区五险基数独立）
 * tax：一级个税区仅摘要 + 查看明细入口（专项附加扣除不在一级主流程）
 * taxDetail：二级面板（专项附加扣除勾选 → 各区间实际税额 → 税率区间表）
 */
export const INCOME_DETAIL_PANEL_ITEMS = {
  ...
  tax: ['预估个税查看明细'],
  taxDetail: ['专项附加扣除勾选', '各区间实际税额', '税率区间表'],
} as const;
