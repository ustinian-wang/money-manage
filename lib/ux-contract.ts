 ...
 *
 * 1. 按密度分档 tip / field / panel；仅 panel 默认走移动全屏内页。
 * 2. ≤2 个控件、无表无图 → 不用全屏感 sheet：居中矮 dialog 或内容贴合底卡；禁止仅为说明文打开 modal sheet。
 * 3. InfoTip：PC 悬停；移动贴锚点气泡（点页面其它处 / 遮罩 / 再点 ? / Esc 关闭），不锁页面滚动。遮罩与气泡 portal 到 body。
 * 4. 移动禁止浮层叠浮层：二级编辑在父 sheet 内 push 子页（返回/Esc 先 pop），不新开 FloatPanel。
 * 5. Panel 全屏内页用于需沉浸或纵向滚动的任务：分析图表、多字段分期、明细表、计划变更；PC 大面板可继续居中+拖拽。
 ...
 * - field：矮底卡 / 内容 hug（无满屏抬升），轻量编辑；点 mask 不关
 * - panel：全屏内页（宽高贴满 VV、顶底左右真边）+ 遮罩 + 顶栏关闭/返回；点 mask 不关；打开时锁 body 滚动
 * - 字段：Settings 式左右 tile（.field-row-mobile）
 * - 支出：卡片（标题+金额 + meta chip），底栏操作
 * - 顶栏吸顶且始终展开；无滚动收起 chrome
